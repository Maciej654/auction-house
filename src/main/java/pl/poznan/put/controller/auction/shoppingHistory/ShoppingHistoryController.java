...
public class ShoppingHistoryController {
    ...
    public void setup(){
        ...
        var query = em.createQuery("select item from ShoppingCartItem item where item.buyer = :buyer and item.auction.status = :status", ShoppingCartItem.class);
    ...
    private String extractDestination(Auction auction) {
        if(em == null){ return "";}
        var query = em.createQuery("select log from AuctionLog  log where log.auction = :auction", AuctionLog.class);
        query.setParameter("auction", auction);
        String desc = query.getResultStream()
