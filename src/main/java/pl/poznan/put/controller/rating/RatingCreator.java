...
public class RatingCreator {
    ...
    private void initialize(){
        ...
        user = Objects.requireNonNull(entityManager).find(User.class, "jacek.hercog@gmail.com");//toDO get the User from login view
    ...
    public void buttonClicked(ActionEvent actionEvent) {
        ...
        Auction auction = shoppingCartItemStream().map(ShoppingCartItem::getAuction)
                .filter(a -> auctionAttributes(a).equals(value)).findAny().get();
    ...
    private Stream<ShoppingCartItem> shoppingCartItemStream(){
        TypedQuery<ShoppingCartItem> query =  Objects.requireNonNull(entityManager)
                .createQuery("select item from ShoppingCartItem item where item.buyer = :buyer and item.auction.status = :status", ShoppingCartItem.class);
